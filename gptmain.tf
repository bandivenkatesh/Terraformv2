...
resource "null_resource" "configure_jump_server" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "sudo -u bandi mkdir -p /home/bandi/.ssh",
      "sudo -u bandi ssh-keygen -t rsa -b 4096 -f /home/bandi/.ssh/id_rsa -N ''",
      "sudo apt-get update",
      "sudo apt-get install -y sshpass",
      "sudo -u bandi sshpass -p 'test' ssh-copy-id -i /home/bandi/.ssh/id_rsa.pub testuser@${google_compute_instance.test_server.network_interface[0].access_config[0].nat_ip}",
      "sudo -u bandi sshpass -p 'pass' ssh-copy-id -i /home/bandi/.ssh/id_rsa.pub produser@${google_compute_instance.prod_server.network_interface[0].access_config[0].nat_ip}",
      "echo 'Host test-server\n  Hostname ${google_compute_instance.test_server.network_interface[0].access_config[0].nat_ip}\n  User testuser\n  IdentityFile ~/.ssh/id_rsa' | sudo tee -a /home/bandi/.ssh/config",
      "echo 'Host prod-server\n  HostName ${google_compute_instance.prod_server.network_interface[0].access_config[0].nat_ip}\n  User produser\n  IdentityFile ~/.ssh/id_rsa' | sudo tee -a /home/bandi/.ssh/config",
      "sudo chown -R bandi:bandi /home/bandi/.ssh",
